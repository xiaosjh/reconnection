...
;;tresp0=aia_get_response(/temp, /dn,/evenorm)
;
...
;
  ...
  ; 直接使用 AIA 温度响应，不调用 XRT
  tresp0 = aia_get_response(/temp, /dn, /evenorm)  ; 改成 /evenorm 而不是 /chiantifix

  if ~isa(tresp0, 'STRUCT') then begin
    print, 'ERROR: Failed to get AIA temperature response'
    tresp = 0
    return
  endif
  ...
  units0 = 'DN cm^5 s^-1 pix^-1'
  descr0 = ' Provides temperature response function for an AIA channel, for a given plasma emission model.'
  comm0 = 'from aia_get_response; AIA only'
  typ0 = 'AIA_TEMP_RESP'
  chan0 = tresp0.channels
  dat0 = tresp0.all

  ; 定义温度网格
  logtx = findgen(26) * 0.1 + 5.5  ; 5.5 to 8.0

  for j = 0, nc-1 do begin
    trespj = {type: typ0, $
      name:chan0[j], $
      temp:logtx, $
      temp_resp:interpol(reform(dat0[*,j]), 10.^logte, logtx), $
      temp_resp_units:units0, $
      confg:{name:chan0[j]}, $
      length:lenx, $
      history:hist0, $
      comments:strarr(5)}
