...
data=np.load(r'D:\Learning\PHD1st\magnetic_reconnecion\program\data_chase.npy')
...
#里面从32到98的34个时刻的数据
fig, ax = plt.subplots()
ax.imshow(data[:, :, 0],cmap='afmhot',origin='lower',vmin=0,vmax=2000)  # 参考图，方便画曲线
clicked_points = []
...
C_y = A_y + shrink_distance * ny
C_x = A_x + shrink_distance * nx
...
for t in range(data.shape[2]):
    plt.imshow(data[:, :, t],cmap='afmhot',origin='lower',vmin=0,vmax=2000)
    ...
    plt.axis('off')
    plt.savefig(r'D:\Learning\PHD1st\magnetic_reconnecion\data\CHASE_Ha_image\red_slice\\'+str(t)+'.png')
...
np.savez(r'D:\Learning\PHD1st\magnetic_reconnecion\data\CHASE_Ha_image\fit_points.npz',
...
np.save(r'D:\Learning\PHD1st\magnetic_reconnecion\data\CHASE_Ha_image\result_matrix.npy',result_matrix)
